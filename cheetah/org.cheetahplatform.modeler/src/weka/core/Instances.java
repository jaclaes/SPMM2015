 ...
 * <p>
 * 
 * All methods that change a set of instances are safe, ie. a change of a set of instances does not affect any other sets of instances. All
 * methods that change a datasets's attribute information clone the dataset before it is changed.
 * 
...
public class Instances implements Serializable, RevisionHandler {
	...
	/**
	 * Merges two sets of Instances together. The resulting set will have all the attributes of the first set plus all the attributes of the
	...
	/*
	 * public invariant (\forall int i; 0 <= i && i < m_Attributes.size(); m_Attributes.elementAt(i) != null);
	 */
	...
	/**
	 * The lines read so far in case of incremental loading. Since the StreamTokenizer will be re-initialized with every instance that is
	 * read, we have to keep track of the number of lines read so far.
	 * 
	...
	/**
	 * Constructor creating an empty set of instances. Copies references to the header information from the given set of instances. Sets the
	...
	/**
	 * Reads an ARFF file from a reader, and assigns a weight of one to each instance. Lets the index of the class attribute be undefined
	 * (negative).
	 * 
	 * @param reader
	 *            the reader
	...
	/**
	 * Reads the header of an ARFF file from a reader and reserves space for the given number of instances. Lets the class index be
	 * undefined (negative).
	 * 
	 * @param reader
	 *            the reader
	 ...
	 *            the capacity
	 * @throws IllegalArgumentException
	 *             if the header is not read successfully or the capacity is negative.
	 * @throws IOException
	 *             if there is a problem with the reader.
	 * @deprecated instead of using this method in conjunction with the <code>readInstance(Reader)</code> method, one should use the
	 *             <code>ArffLoader</code> or <code>DataSource</code> class instead.
	 * @see weka.core.converters.ArffLoader
	 * @see weka.core.converters.DataSource
	 */
	// @ requires capacity >= 0;
	...
	/**
	 * Creates an empty set of instances. Uses the given attribute information. Sets the capacity of the set of instances to 0 if its
	 * negative. Given attribute information must not be changed after this constructor has been used.
	 * 
	 * @param name
	...
	/**
	 * Adds one instance to the end of the set. Shallow copies instance before it is added. Increases the size of the dataset if it is not
	 * large enough. Does not check if the instance is compatible with the dataset. Note: String or relational values are not transferred.
	 * 
	 * @param instance
	...
	/**
	 * Returns an attribute given its name. If there is more than one attribute with the same name, it returns the first one. Returns null
	 * if the attribute can't be found.
	 * 
	 * @param name
	 *            the attribute's name
	 * @return the attribute with the given name, null if the attribute can't be found
	 */
	...
	/**
	 * Gets the value of all instances in this dataset for a particular attribute. Useful in conjunction with Utils.sort to allow iterating
	 * through the dataset in sorted order for some attribute.
	 * 
	 * @param index
	 *            the index of the attribute.
	 * @return an array containing the value of the desired attribute for each instance in the dataset.
	 */
	// @ requires 0 <= index && index < numAttributes();
	...
	/**
	 * Checks if the given instance is compatible with this dataset. Only looks at the size of the instance and the ranges of the values for
	...
	/**
	 * Deletes all attributes of the given type in the dataset. A deep copy of the attribute information is performed before an attribute is
	 * deleted.
	 * 
	 * @param attType
	 ...
	 * 
	 * @throws IllegalArgumentException
	 *             if string attribute couldn't be successfully deleted (probably because it is the class attribute).
	...
	/**
	 * Returns a random number generator. The initial seed of the random number generator depends on the given seed and the hash code of a
	 * string representation of a instances chosen based on the given seed.
	 * 
	 * @param seed
	 *            the given seed
	...
	/**
	 * Inserts an attribute at the given position (0 to numAttributes()) and sets all values to be missing. Shallow copies the attribute
	 * before it is inserted, and performs a deep copy of the existing attribute information.
	 * 
	 * @param att
	...
	/**
	 * Returns the kth-smallest attribute value of a numeric attribute. Note that calling this method will change the order of the data!
	 * 
	 * @param att
	...
	/**
	 * Returns the kth-smallest attribute value of a numeric attribute. Note that calling this method will change the order of the data! The
	 * number of non-missing values in the data must be as least as last as k for this to work.
	 * 
	 * @param attIndex
	...
	/**
	 * Returns the mean (mode) for a numeric (nominal) attribute as a floating-point value. Returns 0 if the attribute is neither nominal
	 ...
	 * 
	 * @return the number of class labels as an integer if the class attribute is nominal, 1 otherwise.
	 * @throws UnassignedClassException
	 *             if the class is not set
	 */
	// @ requires classIndex() >= 0;
	...
	/**
	 * Returns the number of distinct values of a given attribute. Returns the number of instances if the attribute is a string attribute.
	 ...
	 *            the attribute's index (index starts with 0)
	 * @param left
	 *            the first index of the subset to be sorted (index starts with 0)
	 * @param right
	...
	/**
	 * Reads a single instance from the reader and appends it to the dataset. Automatically expands the dataset if it is not large enough to
	 * hold the instance. This method does not check for carriage return at the end of the line.
	 * 
	 * @param reader
	 *            the reader
	...
	/**
	 * Creates a new dataset of the same size using random sampling with replacement according to the current instance weights. The weights
	 * of the instances in the new dataset are set to one.
	 * 
	 * @param random
	 *            a random number generator
	 * @return the new dataset
	...
	/**
	 * Creates a new dataset of the same size using random sampling with replacement according to the given weight vector. The weights of
	 * the instances in the new dataset are set to one. The length of the weight vector has to be the same as the number of instances in the
	 * dataset, and all weights have to be positive.
	 * 
	 * @param random
	 *            a random number generator
	 * @param weights
	...
	/**
	 * Sorts the instances based on an attribute. For numeric attributes, instances are sorted into ascending order. For nominal attributes,
	 * instances are sorted based on the attribute label ordering specified in the header. Instances with missing values for the attribute
	...
	/**
	 * Sorts the instances based on an attribute. For numeric attributes, instances are sorted in ascending order. For nominal attributes,
	 * instances are sorted based on the attribute label ordering specified in the header. Instances with missing values for the attribute
	 * are placed at the end of the dataset.
	 * 
	 * @param attIndex
	...
	/**
	 * Stratifies a set of instances according to its class values if the class attribute is nominal (so that afterwards a stratified
	...
	/**
	 * Create a copy of the structure if the data has string or relational attributes, "cleanses" string types (i.e. doesn't contain
	 * references to the strings seen in the past) and all relational attributes.
	 * 
	...
	/**
	 * Returns the instances in the dataset as a string in ARFF format. Strings are quoted if they contain whitespace characters, or if they
	 * are a question mark.
	 * 
	...
	/**
	 * Returns the dataset as a string in ARFF format. Strings are quoted if they contain whitespace characters, or if they are a question
	 * mark.
	 * 
	 * @return the dataset in ARFF format as a string
	 */
	...
	/**
	 * Generates a string summarizing the set of instances. Gives a breakdown for each attribute indicating the number of
	 * missing/discrete/unique values and other information.
	 * 
	...
	/**
	 * Creates the training set for one fold of a cross-validation on the dataset. The data is subsequently randomized based on the given
