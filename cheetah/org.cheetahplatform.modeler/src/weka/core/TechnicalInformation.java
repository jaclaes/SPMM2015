 ...
 * target="_blank">http://www.ecst.csuchico.edu/~jacobsd/bib/formats/bibtex.html</a>
 * <p/>
...
public class TechnicalInformation implements RevisionHandler {
	...
	public enum Field {
		/**
		 * Usually the address of the publisher or other type of institution. For major publishing houses, van Leunen recommends omitting
		 * the information entirely. For small publishers, on the other hand, you can help the reader by giving the complete address.
		 */
		ADDRESS(
				"address",
		...
		/**
		 * An annotation. It is not used by the standard bibliography styles, but may be used by others that produce an annotated
		 * bibliography.
		 */
		ANNOTE("annote",
		...
		/**
		 * Title of a book, part of which is being cited. See the LaTeX book for how to type titles. For book entries, use the title field
		 * instead.
		 */
		BOOKTITLE("booktitle",
		...
		/**
		 * The database key of the entry being cross referenced. Any fields that are missing from the current record are inherited from the
		...
		/**
		 * The edition of a book---for example, ``Second''. This should be an ordinal, and should have the first letter capitalized, as
		...
		/**
		 * Name(s) of editor(s), typed as indicated in the LaTeX book. If there is also an author field, then the editor field gives the
		...
		/**
		 * Used for alphabetizing, cross referencing, and creating a label when the ``author'' information is missing. This field should not
		 * be confused with the key that appears in the cite command and at the beginning of the database entry.
		 */
		KEY(
				"key",
		...
		/**
		 * The month in which the work was published or, for an unpublished work, in which it was written. You should use the standard
		 * three-letter abbreviation, as described in Appendix B.1.3 of the LaTeX book.
		 */
		MONTH(
				"month",
		...
		/**
		 * The number of a journal, magazine, technical report, or of a work in a series. An issue of a journal or magazine is usually
		 * identified by its volume and number; the organization that issues a technical report usually gives it a number; and sometimes
		...
		/**
		 * One or more page numbers or range of numbers, such as 42--111 or 7,41,73--97 or 43+ (the `+' in this last example indicates pages
		 * following that don't form a simple range). To make it easier to maintain Scribe-compatible databases, the standard styles convert
		...
		/**
		 * The name of a series or set of books. When citing an entire book, the the title field gives its title and an optional series
		...
		/**
		 * The year of publication or, for an unpublished work, the year it was written. Generally it should consist of four numerals, such
		 * as 1984, although the standard styles can handle any year whose last four nonpunctuation characters are numerals, such as
		 * `\\hbox{(about 1984)}'.
		 */
		YEAR(
				"year",
		...
		/**
		 * The WWW Universal Resource Locator that points to the item being referenced. This often is used for technical reports to point to
		 ...
		 * 
		 * @param display
		 *            the string to return in toString()
		 * @param comment
	...
	protected Hashtable<Object, String> m_Values = new Hashtable<Object, String>();

	/** additional technical informations */
	protected Vector<Object> m_Additional = new Vector<Object>();
	 ...
	 *            the type of this information
	 * @param id
	 *            the unique ID (for BibTex), can be empty
	 * @see Type
	 */
	 ...
	 * 
	 * @param type
	 *            the type of the new information to add
	 ...
	 */
	public Enumeration<Object> additional() {
	 ...
	 */
	public Enumeration<Object> fields() {
	...
	/**
	 * Returns a plain-text string representing this technical information. Note: it only returns a string based on some fields. At least
	 * AUTHOR, YEAR and TITLE are necessary.
	 * 
