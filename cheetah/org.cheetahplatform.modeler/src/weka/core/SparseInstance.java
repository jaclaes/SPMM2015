...
/**
 * Class for storing an instance as a sparse vector. A sparse instance only requires storage for those attribute values that are non-zero.
 * Since the objective is to reduce storage requirements for datasets with large numbers of default values, this also includes nominal
 * attributes -- the first nominal value (i.e. that which has index 0) will not require explicit storage, so rearrange your nominal
 * attribute value orderings if necessary. Missing values will be stored explicitly.
 * 
 * @author Eibe Frank
...
public class SparseInstance extends Instance {
	...
	/**
	 * Constructor that generates a sparse instance from the given parameters. Reference to the dataset is set to null. (ie. the instance
	 ...
	 * have access to information about the attribute types) Note that the indices need to be sorted in ascending order. Otherwise things
	 * won't work properly.
	 * 
	 * @param weight
	 *            the instance's weight
	 ...
	 * @param indices
	 *            the indices of the given values in the full vector (need to be sorted in ascending order)
	 * @param maxNumValues
	...
	/**
	 * Constructor that generates a sparse instance from the given instance. Reference to the dataset is set to null. (ie. the instance
	 * doesn't have access to information about the attribute types)
	 * 
	 * @param instance
	...
	/**
	 * Constructor that copies the info from the given instance. Reference to the dataset is set to null. (ie. the instance doesn't have
	 ...
	 * access to the dataset, use <code>new SparseInstance(instance)</code>
	 * 
	...
	/**
	 * Returns the description of one instance in sparse format. If the instance doesn't have access to a dataset, it returns the internal
	 * floating-point values. Quotes string values that contain whitespace characters.
	 * 
			...
			if (isMissingValue(m_AttValues[i])) {
				...
				if (m_Dataset == null) {
					...
					if (m_Dataset.attribute(m_Indices[i]).isNominal() || m_Dataset.attribute(m_Indices[i]).isString()
						...
						try {
							text.append(m_Indices[i] + " "
									+ Utils.quote(m_Dataset.attribute(m_Indices[i]).relation((int) valueSparse(i)).stringWithoutHeader()));
						} catch (Exception e) {
							e.printStackTrace();
							System.err.println(new Instances(m_Dataset, 0));
							System.err.println("Att:" + m_Indices[i] + " Val:" + valueSparse(i));
