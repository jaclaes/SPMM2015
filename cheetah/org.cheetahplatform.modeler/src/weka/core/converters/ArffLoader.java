...
/**
 * <!-- globalinfo-start --> Reads a source that is in arff (attribute relation file format) format.
 * <p/>
 * <!-- globalinfo-end -->
 * 
...
public class ArffLoader extends AbstractFileLoader implements BatchConverter, IncrementalConverter, URLSourcedLoader {
	...
	public static class ArffReader implements RevisionHandler {
		...
		/**
		 * Reads the data without header according to the specified template. The data can be accessed via the <code>getData()</code>
		 * method.
		 * 
		 * @param reader
		 *            the reader to use
		...
		/**
		 * Initializes the reader without reading the header according to the specified template. The data must be read via the
		...
		/**
		 * Reads only the header and reserves the specified space for instances. Further instances can be read via
		 * <code>readInstance()</code>.
		 * 
		 * @param reader
		 *            the reader to use
		 * @param capacity
		 ...
		 * 
		 * @param structure
		 *            the dataset header information, will get updated in case of string or relational attributes
		 * @param flag
		...
		protected Instance getInstanceSparse(boolean flag) throws IOException {
			...
			do {
				...
				if (m_Tokenizer.ttype == '?') {
					...
					case Attribute.RELATIONAL:
						try {
							ArffReader arff = new ArffReader(new StringReader(m_Tokenizer.sval), m_Data.attribute(
									m_IndicesBuffer[numValues]).relation(), 0);
							Instances data = arff.getData();
		 ...
		 * 
		 * @return the value of the instance's weight, or NaN if no weight has been supplied in the file
		 */
					...
					do {
						if (Attribute.ARFF_ATTRIBUTE.equalsIgnoreCase(m_Tokenizer.sval)) {
							...
							if (!attributeName.equalsIgnoreCase(m_Tokenizer.sval)) {
								errorMessage("declaration of subrelation " + attributeName + " must be terminated by " + "@end "
										+ attributeName);
							}
							break;
						} else {
	...
	/**
	 * Return the full data set. If the structure hasn't yet been determined by a call to getStructure then method should do so before
	 * processing the rest of the data set.
	 * 
	...
	/**
	 * Read the data set incrementally---get the next instance in the data set or returns null if there are no more instances to get. If the
	 * structure hasn't yet been determined by a call to getStructure then method should do so before returning the next instance in the
	 * data set.
	 * 
	 * @param structure
	 *            the dataset header information, will get updated in case of string or relational attributes
	 * @return the next instance in the data set as an Instance object or null if there are no more instances to be read
	 * @throws IOException
	 ...
	 * 
	 * @return a description of the Loader suitable for displaying in the explorer/experimenter gui
	 */
