...
public final class ShortestPathConnectionRouter extends AbstractRouter {
	...
	/**
	 * @return All connection paths after routing dirty paths. Some of the paths that were not dirty may change as well, as a consequence of
	 *         new routings.
	 * @since 3.5
	 */
	...
	public void route(Connection conn) {
		if (isDirty) {
			...
			for (int i = 0; i < updated.size(); i++) {
				...
				ConnectionAnchor anchor = current.getSourceAnchor();

				System.out.println("Source");
				System.out.println("-Anchor: " + anchor);
				System.out.println("-Ref1: " + ref1);

				Point p = anchor.getLocation(ref1);
				if (p == null) {
					System.out.println(">NulllPointerException");
					anchor.getLocation(ref1);
				}
				start = p.getCopy();
				ConnectionAnchor anchor2 = current.getTargetAnchor();

				System.out.println("Target");
				System.out.println("-Anchor2: " + anchor);
				System.out.println("-Ref2: " + ref1);

				Point p2 = anchor2.getLocation(ref2);
				if (p2 == null) {
					System.out.println(">NulllPointerException");
					anchor2.getLocation(ref2);
				}
				end = p2.getCopy();
