...
/**
 * This strategy does not take the structure of a process into account but the occurrence of the single activities.
 * 
 * @author Christian Haisjackl
 */
public class MultiSetStrategy extends Strategy {
	private Miner miner;

	/**
	 * Creates a Multi Set Strategy.
	 * 
	 * @param log
	 *            The log to use
	 * @param target
	...
	protected MultiSetStrategy(Log log, TargetFunction target, String eventState, List<Object> additionalParams) {
		super(log, target, eventState);
		try {
			miner = Miner.getInstance("log.miner.MultiSetMiner", this);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
