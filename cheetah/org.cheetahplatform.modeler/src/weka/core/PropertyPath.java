...
/**
 * A helper class for accessing properties in nested objects, e.g., accessing the "getRidge" method of a LinearRegression classifier part of
 * MultipleClassifierCombiner, e.g., Vote. For doing so, one needs to supply the object to work on and a property path. The property path is
 * a dot delimited path of property names ("getFoo()" and "setFoo(int)" have "foo" as property name), indices of arrays are 0-based. E.g.:
 * <p/>
 * 
 ...
 * of the given Vote meta-classifier and there the property descriptor of the "ridge" property. <code>getValue(...)</code> will return the
 * actual value of the ridge parameter and <code>setValue(...)</code> will set it.
 * 
...
public class PropertyPath implements RevisionHandler {
	...
	public static class Path implements RevisionHandler {
		...
		public Path(Vector<PathElement> elements) {
			...
			for (int i = 0; i < elements.size(); i++)
				m_Elements.add(elements.get(i).clone());
		}
		...
		/**
		 * returns a subpath of the current structure, starting with the specified element index up. The endIndex specifies the element that
		 * is not part of the new subpath. In other words, the new path contains the elements from "startIndex" up to "(endIndex-1)".
		 * 
		 * @param startIndex
		 *            the first element of the subpath
		 * @param endIndex
	...
	public static void main(String[] args) throws Exception {
		...
		/*
		 * // retrieving ridge with path weka.classifiers.meta.Vote vote = new weka.classifiers.meta.Vote(); vote.setClassifiers( new
		 * weka.classifiers.Classifier[]{ new weka.classifiers.trees.J48(), new weka.classifiers.functions.LinearRegression()}); path = new
		 * Path("classifiers[1].ridge"); System.out.println("path: " + path + " -> " + getValue(vote, path));
		 * 
		 * // setting ridge with path and retrieving it again setValue(vote, path.toString(), new Double(0.1)); System.out.println("path: "
		 * + path + " -> " + getValue(vote, path));
		 */
	}
