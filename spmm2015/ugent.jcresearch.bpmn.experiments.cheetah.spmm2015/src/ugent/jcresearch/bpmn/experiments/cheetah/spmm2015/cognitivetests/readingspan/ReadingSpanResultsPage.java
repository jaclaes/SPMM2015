...
public class ReadingSpanResultsPage extends WizardPage implements Listener {
	protected List<ReadingSpanExercise> level;
	...
	public ReadingSpanResultsPage(String pageName, List<ReadingSpanExercise> level, int num, int totalNum) {
		super(pageName);
		this.level = level;
		this.textField = null;
		this.num = num;
	...
	public AuditTrailEntry createInputChangedAuditTrailEntry(String eq, String word, String wordEntered) {
		AuditTrailEntry entry = new AuditTrailEntry(INPUT_CHANGED);
		entry.setAttribute("isDemo", isDemo());
		entry.setAttribute("equation", eq);
	...
	public void logInitialStatus() {
		for (ReadingSpanExercise ex : level) {
