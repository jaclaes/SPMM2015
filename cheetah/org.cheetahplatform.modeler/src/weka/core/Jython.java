...
public class Jython implements Serializable, RevisionHandler {
	 ...
	 * 
	 * @param o
	 *            the object the method should be called from, e.g., a Python Interpreter
	 * @param methodName
	 ...
	 * 
	 * @param filename
	 *            the path to the Jython module, incl. filename
	 * @param template
	...
	/**
	 * loads the module and returns a new instance of it as instance of the provided Java class template. The paths are added to 'sys.path'
	...
	public static Object newInstance(File file, Class<?> template, File[] paths) {
		...
		if (paths.length > 0) {
			...
			try {
				tmpPaths = (String[]) invoke(interpreter, "get", new Class[] { String.class, Class.class }, new Object[] { instanceName,
						String[].class });
