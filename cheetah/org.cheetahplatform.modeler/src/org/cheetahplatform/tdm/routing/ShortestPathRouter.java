package org.cheetahplatform.tdm.routing;
...
/**
 * Bends a collection of {@link Path Paths} around rectangular obstacles. This class maintains a list of paths and obstacles. Updates can be
 * made to the paths and/or obstacles, and then an incremental solve can be invoked.
 * <P>
 * The algorithm will attempt to find the shortest non-intersecting path between each path's start and end points. Once all paths have been
 * found, they will be offset based on how many paths bend around the same corner of each obstacle.
 * <P>
 * The worst-case performance of this algorithm is p * s * n^2, where p is the number of paths, n is the number of obstacles, and s is the
 * average number of segments in each path's final solution.
 * <P>
...
public class ShortestPathRouter {
	...
	/**
	 * The number of times to grow obstacles and test for intersections. This is a tradeoff between performance and quality of output.
	 */
	...
	/**
	 * Resyncs the parent paths with any new child paths that are necessary because bendpoints have been added to the parent path.
	 * 
	 * private void generateChildPaths() { for (int i = 0; i < userPaths.size(); i++) { Path path = (Path)userPaths.get(i); PointList
	 * bendPoints = path.bendpoints; if (bendPoints != null && bendPoints.size() != 0) { List childPaths = new ArrayList(bendPoints.size() +
	 * 1); Path child = null; Vertex prevVertex = path.start; Vertex currVertex = null;
	 * 
	 * for (int b = 0; b < bendPoints.size(); b++) { Point bp = (Point)bendPoints.getPoint(b); currVertex = new Vertex(bp, null); child =
	 * new Path(prevVertex, currVertex); childPaths.add(child); workingPaths.add(child); prevVertex = currVertex; }
	 * 
	 * child = new Path(prevVertex, path.end); childPaths.add(child); workingPaths.add(child); pathsToChildPaths.put(path, childPaths); }
	 * else workingPaths.add(path); } //End FOR }
	 */
	...
	/**
	 * Performs a single pass of the grow obstacles step, this can be repeated as desired. Grows obstacles, then tests paths against the
	 * grown obstacles.
	 */
	...
	/**
	 * Labels the given path's vertices as innies, or outies, as well as determining if this path is inverted.
	 * 
	 * @param path
	 *            the path
	 */
	 ...
	 * @param crossProduct
	 *            the cross product of this segment and a segment to the obstacles center
	 * @param path
	 *            the path
	 */
	...
	/**
	 * Populates the parent paths with all the child paths that were created to represent bendpoints.
	 */
	...
	/**
	 * Sets the default spacing between paths. The spacing is the minimum distance that path should be offset from other paths or obstacles.
	 * The default value is 4. When this value can not be satisfied, paths will be squeezed together uniformly.
	 * 
	 * @param spacing
	 *            the path spacing
	 * @since 3.2
	...
	private int testOffsetSegmentForIntersections(Segment segment, int index, Path path) {
		for (int i = 0; i < userObstacles.size(); i++) {
			...
			if (segment.getSlope() < 0) {
				if (segment.intersects(obs.topLeft.x - offset, obs.topLeft.y - offset, obs.bottomRight.x + offset, obs.bottomRight.y
						+ offset))
					vertex = getNearestVertex(obs.topLeft, obs.bottomRight, segment);
				else if (segment.intersects(obs.bottomLeft.x - offset, obs.bottomLeft.y + offset, obs.topRight.x + offset, obs.topRight.y
						- offset))
					vertex = getNearestVertex(obs.bottomLeft, obs.topRight, segment);
			} else {
				if (segment.intersects(obs.bottomLeft.x - offset, obs.bottomLeft.y + offset, obs.topRight.x + offset, obs.topRight.y
						- offset))
					vertex = getNearestVertex(obs.bottomLeft, obs.topRight, segment);
				else if (segment.intersects(obs.topLeft.x - offset, obs.topLeft.y - offset, obs.bottomRight.x + offset, obs.bottomRight.y
						+ offset))
					vertex = getNearestVertex(obs.topLeft, obs.bottomRight, segment);
			}
