...
/**
 * A general purpose class for parsing mathematical expressions involving attribute values. Values can be provided in an array or in an
 * Instance. Values are accessed in the expression by prefixing their index (starting at 1) with the character 'a'.
 * 
 * <pre>
...
public class AttributeExpression implements Serializable, RevisionHandler {
	 ...
	 */
	private class AttributeOperand implements Serializable, RevisionHandler {
	 ...
	 */
	private class NumericOperand implements Serializable, RevisionHandler {
	 ...
	 */
	private class Operator implements Serializable, RevisionHandler {
	...
	/**
	 * Unary functions. l = log, b = abs, c = cos, e = exp, s = sqrt, f = floor, h = ceil, r = rint, t = tan, n = sin
	 */
	...
	private Vector<RevisionHandler> m_postFixExpVector;
	...
	/**
	 * Converts a string containing a mathematical expression in infix form to postfix form. The result is stored in the vector
	 * m_postfixExpVector
	 * 
	 * @param infixExp
	...
	public void convertInfixToPostfix(String infixExp) throws Exception {
		...
		m_postFixExpVector = new Vector<RevisionHandler>();
		...
		while (!m_operatorStack.empty()) {
			String popop = (m_operatorStack.pop());
	...
	/**
	 * Evaluate the expression using the supplied array of attribute values. The result is stored in the last element of the array. Assumes
	 * that the infix expression has been converted to postfix and stored in m_postFixExpVector
	 * 
	 * @param vals
	...
	public void evaluateExpression(double[] vals) throws Exception {
		...
		for (int i = 0; i < m_postFixExpVector.size(); i++) {
			...
			if (nextob instanceof NumericOperand) {
				...
				/*
				 * if (Instance.isMissingValue(value)) { vals[vals.length-1] = Instance.missingValue(); break; }
				 */
				...
				if (isUnaryFunction(op)) {
					double operand = operands.pop().doubleValue();
					double result = ((Operator) nextob).applyFunction(operand);
					operands.push(new Double(result));
				} else {
					double second = operands.pop().doubleValue();
					double first = operands.pop().doubleValue();
	...
	/**
	 * Evaluate the expression using the supplied Instance. Assumes that the infix expression has been converted to postfix and stored in
	 * m_postFixExpVector
	 * 
	 * @param instance
	 *            the Instance containing values to apply the expression to
	 * @throws Exception
	 *             if something goes wrong
	 */
	...
	private void handleOperator(String tok) throws Exception {
		...
		if (tokchar == ')') {
			...
			while (!m_operatorStack.empty() && stackPriority((m_operatorStack.peek()).charAt(0)) >= infixToc) {
