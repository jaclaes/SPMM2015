 ...
 * 
 * Copyright 2001 University of Waikato Copyright 1999 CERN - European Organization for Nuclear Research. Permission to use, copy, modify,
 ...
 * notice appear in all copies and that both that copyright notice and this permission notice appear in supporting documentation. CERN and
 * the University of Waikato make no representations about the suitability of this software for any purpose. It is provided "as is" without
...
public class Statistics implements RevisionHandler {
	...
	protected static final double P0[] = { -5.99633501014107895267E1, 9.80010754185999661536E1, -5.66762857469070293439E1,
	...
	/**
	 * Returns chi-squared probability for given value and degrees of freedom. (The probability that the chi-squared variate will be greater
	 * than x for the given degrees of freedom.)
	 * 
	 * @param x
	 *            the value
	 * @param v
	 ...
	 * </pre>
	 * 
	 * <b>Implementation:</b> For <tt>0 <= |x| < 1, erf(x) = x * P4(x**2)/Q5(x**2)</tt>; otherwise <tt>erf(x) = 1 - erfc(x)</tt>.
	 * <p>
	 * Code adapted from the <A HREF="http://www.sci.usq.edu.au/staff/leighb/graph/Top.html"> Java 2D Graph Package 2.4</A>, which in turn
	 * is a port from the <A HREF="http://people.ne.mediaone.net/moshier/index.html#Cephes">Cephes 2.2</A> Math Library (C).
	 * 
	 * @param a
	...
	public static double errorFunctionComplemented(double a) {
		...
		double P[] = { 2.46196981473530512524E-10, 5.64189564831068821977E-1, 7.46321056442269912687E0, 4.86371970985681366614E1,
				1.96520832956077098242E2, 5.26445194995477358631E2, 9.34528527171957607540E2, 1.02755188689515710272E3,
	...
	/**
	 * Returns the value, <tt>x</tt>, for which the area under the Normal (Gaussian) probability density function (integrated from minus
	 * infinity to <tt>x</tt>) is equal to the argument <tt>y</tt> (assumes mean is zero, variance is one).
	 * <p>
	 * For small arguments <tt>0 < y < exp(-2)</tt>, the program computes <tt>z = sqrt( -2.0 * log(y) )</tt>; then the approximation is
	 * <tt>x = z - log(z)/z  - (1/z) P(1/z) / Q(1/z)</tt>. There are two rational functions P/Q, one for <tt>0 < y < exp(-32)</tt> and the
	 * other for <tt>y</tt> up to <tt>exp(-2)</tt>. For larger arguments, <tt>w = y - 0.5</tt>, and
	...
	/**
	 * Returns the area under the Normal (Gaussian) probability density function, integrated from minus infinity to <tt>x</tt> (assumes mean
	 * is zero, variance is one).
	 * 
	 * <pre>
	 *                            x
	 *                             -
	 ...
	 * </pre>
	 * 
	 * where <tt>z = x/sqrt(2)</tt>. Computation is via the functions <tt>errorFunction</tt> and <tt>errorFunctionComplement</tt>.
	 * 
	 * @param a
	 *            the z-value
	...
	/**
	 * Evaluates the given polynomial of degree <tt>N</tt> at <tt>x</tt>. Evaluates polynomial when coefficient of N is 1.0. Otherwise same
	 * as <tt>polevl()</tt>.
	 * 
	 * <pre>
	 *                     2          N
	 ...
	 * </pre>
	 * 
	 * The function <tt>p1evl()</tt> assumes that <tt>coef[N] = 1.0</tt> and is omitted from the array. Its calling arguments are otherwise
	 * the same as <tt>polevl()</tt>.
	 * <p>
