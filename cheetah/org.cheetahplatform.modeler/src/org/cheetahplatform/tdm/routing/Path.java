...
/**
 * A Path representation for the ShortestPathRouting. A Path has a start and end point and may have bendpoints. The output of a path is
 * accessed via the method <code>getPoints()</code>.
 * 
...
public class Path {
	...
	/**
	 * The previous cost ratio of the path. The cost ratio is the actual path length divided by the length from the start to the end.
	 */
	...
	/**
	 * Attempts to add a segment between the given obstacles to the visibility graph. This method is specifically written for the case where
	 * the two obstacles intersect and contains a boolean as to whether to check the diagonal that includes the top right point of the other
	 * obstacle.
	 * 
	 * @param segment
	 ...
	 *            whether or not to check the diagonal containing top right point
	 */
	private void addConnectingSegment(Segment segment, Obstacle o1, Obstacle o2, boolean checkTopRight1, boolean checkTopRight2) {
		if (threshold != 0
				&& (segment.end.getDistance(end) + segment.end.getDistance(start) > threshold || segment.start.getDistance(end)
	...
	/**
	 * Attempts to add a segment to the visibility graph. First checks to see if the segment is outside the threshold oval. Then it compares
	 * the segment against all obstacles. If it is clean, the segment is finally added to the graph.
	 * 
	 * @param segment
	 *            the segment
	 * @param exclude1
	...
	private void addSegment(Segment segment, Obstacle exclude1, Obstacle exclude2, List allObstacles) {
		...
		for (int i = 0; i < allObstacles.size(); i++) {
			...
			if (segment.intersects(obs.x, obs.y, obs.right() - 1, obs.bottom() - 1)
					|| segment.intersects(obs.x, obs.bottom() - 1, obs.right() - 1, obs.y) || obs.containsProper(segment.start)
 ...
 */
	...
	/**
	 * Once the visibility graph is constructed, this is called to label the graph and determine the shortest path. Returns false if no path
	 * can be found.
	 * 
	...
	/**
	 * Creates the visibility graph and returns whether or not a shortest path could be determined.
	 * 
	 * @param allObstacles
	 *            the list of all obstacles
	...
	/**
	 * Returns the list of constrained points through which this path must pass or <code>null</code>.
	 * 
	...
	/**
	 * Resets the vertices that this path has traveled prior to this segment. This is called when the path has become inverted and needs to
	 * rectify any labeling mistakes it made before it knew it was inverted.
	 * 
	 * @param currentSegment
	...
	/**
	 * Called to reconnect a subpath back onto this path. Does a depth-first search to reconnect all paths. Should be called after sorting.
	 */
	...
	/**
	 * Refreshes the exclude field on the obstacles in the list. Excludes all obstacles that contain the start or end point for this path.
	 * 
	 * @param allObstacles
	 *            list of all obstacles
	 */
		...
		for (int i = 0; i < allObstacles.size(); i++) {
			...
			if (o.contains(start)) {
				if (o.containsProper(start))
					...
					/*
					 * $TODO Check for corners. If the path begins exactly at the corner of an obstacle, the exclude should also be true.
					 * 
					 * Or, change segment intersection so that two segments that share an endpoint do not intersect.
					 */
				}
			}
	...
	/**
	 * Returns <code>true</code> if the path is clean and intersects the given obstacle. Also dirties the path in the process.
	 * 
	 * @since 3.0
	 * @param obs
	 *            the obstacle
