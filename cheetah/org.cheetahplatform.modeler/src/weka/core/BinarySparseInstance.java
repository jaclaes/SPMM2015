...
/**
 * Class for storing a binary-data-only instance as a sparse vector. A sparse instance only requires storage for those attribute values that
 * are non-zero. Since the objective is to reduce storage requirements for datasets with large numbers of default values, this also includes
 * nominal attributes -- the first nominal value (i.e. that which has index 0) will not require explicit storage, so rearrange your nominal
 * attribute value orderings if necessary. Missing values are not supported, and will be treated as 1 (true).
 * 
...
public class BinarySparseInstance extends SparseInstance {
	...
	/**
	 * Constructor of an instance that sets weight to one, all values to 1, and the reference to the dataset to null. (ie. the instance
	 * doesn't have access to information about the attribute types)
	 * 
	 * @param numAttributes
