...
/**
 * <!-- globalinfo-start --> Loads all text files in a directory and uses the subdirectory names as class labels. The content of the text
 * files will be stored in a String attribute, the filename can be stored as well.
 * <p/>
 * <!-- globalinfo-end -->
 * 
...
public class TextDirectoryLoader extends AbstractLoader implements BatchConverter, OptionHandler {
	 ...
	 * 
	 * @return the character set name to use (or empty string to indicate that the default character set will be used).
	 */
		...
		for (int k = 0; k < classes.size(); k++) {
			...
			for (int j = 0; j < files.length; j++) {
				try {
					...
					System.err
							.println("failed to convert file: " + directoryPath + File.separator + subdirPath + File.separator + files[j]);
				}
			}
		}
