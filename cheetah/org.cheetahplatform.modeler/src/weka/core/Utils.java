...
public final class Utils implements RevisionHandler {
	...
	/**
	 * Returns the given Array in a string representation. Even though the parameter is of type "Object" one can hand over primitve arrays,
	 * e.g. int[3] or double[2][4].
	 * 
	 * @param array
	...
	/**
	 * Converts a File's absolute path to a path relative to the user (ie start) directory. Includes an additional workaround for Cygwin,
	 * which doesn't like upper case drive letters.
	 * 
	 * @param absolute
	...
	/**
	 * Pads a string to a specified length, inserting spaces as required. If the string is too long, characters are removed (from the
	 * right).
	 * 
	 * @param inString
	 *            the input string
	 ...
	 * implements OptionHandler and the options parameter is non-null, the object will have it's options set. Example use:
	 * <p>
	 * 
	 * <code> <pre>
	 ...
	 * @param options
	 *            an array of options suitable for passing to setOptions. May be null. Any options accepted by the object will be removed
	 *            from the array.
	 * @return the newly created object, ready for use.
	 * @exception Exception
	 *                if the class name is invalid, or if the class is not assignable to the desired class type, or the options supplied are
	 *                not acceptable to the object
	 */
	...
	/**
	 * Returns the dimensions of the given array. Even though the parameter is of type "Object" one can hand over primitve arrays, e.g.
	 * int[3] or double[2][4].
	 * 
	 * @param array
	...
	/**
	 * Checks if the given array contains the flag "-Char". Stops searching at the first marker "--". If the flag is found, it is replaced
	...
	/**
	 * Checks if the given array contains the flag "-String". Stops searching at the first marker "--". If the flag is found, it is replaced
	 * with the empty string.
	 * 
	 * @param flag
	...
	/**
	 * Gets an option indicated by a flag "-Char" from the given array of strings. Stops searching at the first marker "--". Replaces flag
	 * and option with empty strings.
	 * 
	 * @param flag
	...
	/**
	 * Gets an option indicated by a flag "-String" from the given array of strings. Stops searching at the first marker "--". Replaces flag
	 * and option with empty strings.
	 * 
	 * @param flag
	...
	/**
	 * Gets the index of an option or flag indicated by a flag "-String" from the given array of strings. Stops searching at the first
	 * marker "--".
	 * 
	 * @param flag
	 *            the String indicating the option.
	...
	/**
	 * Converts an array containing the natural logarithms of probabilities stored in a vector back into probabilities. The probabilities
	...
	/**
	 * Pads a string to a specified length, inserting spaces on the left as required. If the string is too long, characters are removed
	 * (from the right).
	 * 
	 * @param inString
	 *            the input string
	...
	/**
	 * Pads a string to a specified length, inserting spaces on the right as required. If the string is too long, characters are removed
	 * (from the right).
	 * 
	 * @param inString
	 *            the input string
	...
	/**
	 * Returns the secondary set of options (if any) contained in the supplied options array. The secondary set is defined to be any options
	 * after the first "--". These options are removed from the original options array.
	 * 
	 * @param options
	...
	/**
	 * Rounds a double to the next nearest integer value in a probabilistic fashion (e.g. 0.8 has a 20% chance of being rounded down to 0
	 * and a 80% chance of being rounded up to 1). In the limit, the average of the rounded numbers generated by this procedure should
	 * converge to the original double.
	 * 
	 * @param value
	 *            the double value
	 * @param rand
	 ...
	 * 
	 * A string is enclosed within single quotes if a character has been backquoted using the previous rule above or contains <tt>{ }</tt>
	 * or is exactly equal to the strings <tt>, ? space or ""</tt> (empty string).
	 * 
	...
	/**
	 * Reads properties that inherit from three locations. Properties are first defined in the system resource location (i.e. in the
	 * CLASSPATH). These default properties must exist. Properties defined in the users home directory (optional) override default settings.
	 * Properties defined in the current directory (optional) override all these settings.
	 * 
	 * @param resourceName
	 *            the location of the resource that should be loaded. e.g.: "weka/core/Utils.props". (The use of hardcoded forward slashes
	 *            here is OK - see jdk1.1/docs/guide/misc/resources.html) This routine will also look for the file (in this case)
	 *            "Utils.props" in the users home directory and the current directory.
	...
	public static Properties readProperties(String resourceName) throws Exception {
		...
		try {
			...
			/*
			 * throw new Exception("Problem reading default properties: " + ex.getMessage());
			 */
	...
	/**
	 * Sorts a given array of doubles in ascending order and returns an array of integers with the positions of the elements of the original
	 * array in the sorted array. NOTE THESE CHANGES: the sort is no longer stable and it doesn't use safe floating-point comparisons
	 * anymore. Occurrences of Double.NaN are treated as Double.MAX_VALUE
	 * 
	 * @param array
	...
	/**
	 * Sorts a given array of integers in ascending order and returns an array of integers with the positions of the elements of the
	 * original array in the sorted array. The sort is stable. (Equal elements remain in their original order.)
	 * 
	 * @param array
	 *            this array is not changed by the method!
	 * @return an array of integers with the positions in the sorted array.
	...
	/**
	 * Sorts a given array of doubles in ascending order and returns an array of integers with the positions of the elements of the original
	 * array in the sorted array. The sort is stable (Equal elements remain in their original order.) Occurrences of Double.NaN are treated
	 * as Double.MAX_VALUE
	 * 
	 * @param array
	 *            this array is not changed by the method!
	...
	/**
	 * The inverse operation of backQuoteChars(). Converts back-quoted carriage returns and new lines in a string to the corresponding
	...
	/**
	 * unquotes are previously quoted string (but only if necessary), i.e., it removes the single quotes around it. Inverse to
	 * quote(String).
	 * 
	 * @param string
