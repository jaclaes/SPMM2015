 ...
 * <li>string: <br/>
 * This type of attribute represents a dynamically expanding set of nominal values. Usually used in text classification.</li>
 * <li>date: <br/>
 * This type of attribute represents a date, internally represented as floating-point number storing the milliseconds since January 1, 1970,
 * 00:00:00 GMT. The string representation of the date must be <a href="http://www.iso.org/iso/en/prods-services/popstds/datesandtime.html"
 * target="_blank"> ISO-8601</a> compliant, the default is <code>yyyy-MM-dd'T'HH:mm:ss</code>.</li>
 * <li>relational: <br/>
 * This type of attribute can contain other attributes and is, e.g., used for representing Multi-Instance data. (Multi-Instance data
 * consists of a nominal attribute containing the bag-id, then a relational attribute with all the attributes of the bag, and finally the
 * class attribute.)</li>
 * </ul>
 * 
...
public class Attribute implements Copyable, Serializable, RevisionHandler {
	...
	/*
	 * @ invariant m_Type == NUMERIC || m_Type == DATE || m_Type == STRING || m_Type == NOMINAL || m_Type == RELATIONAL;
	 */
	...
	/**
	 * Constructor for nominal attributes and string attributes. If a null vector of attribute values is passed to the method, the attribute
	...
	/**
	 * Constructor for nominal attributes and string attributes with a particular index. If a null vector of attribute values is passed to
	 * the method, the attribute is assumed to be a string.
	 * 
	 * @param attributeName
	 *            the name for the attribute
	 * @param attributeValues
	...
	/**
	 * Constructor for nominal attributes and string attributes, where metadata is supplied. If a null vector of attribute values is passed
	...
	/*
	 * @ ensures m_Name == attributeName; ensures m_Index == -1; ensures attributeValues == null && m_Type == STRING || attributeValues !=
	 * null && m_Type == NOMINAL && m_Values.size() == attributeValues.size(); signals (IllegalArgumentException ex) (* if duplicate strings
	 * in attributeValues *);
	 */
		...
		if (attributeValues == null) {
			...
			for (int i = 0; i < attributeValues.size(); i++) {
				...
				if (m_Hashtable.containsKey(store)) {
					throw new IllegalArgumentException("A nominal attribute (" + attributeName + ") cannot" + " have duplicate labels ("
							+ store + ").");
				}
	 ...
	 *            the name for the attribute
	 * @param dateFormat
	 *            a string suitable for use with SimpleDateFormatter for parsing dates. Null for a default format string.
	 * @param index
	 *            the attribute's index
	 */
	// @ requires attributeName != null;
	 ...
	 * @param value
	 *            The value to add
	 * @return the index assigned to the value, or -1 if the attribute is not of type Attribute.RELATIONAL
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Adds a string value to the list of valid strings for attributes of type STRING and returns the index of the string. This method is
	 ...
	 * @return the index assigned to the string, or -1 if the attribute is not of type Attribute.STRING
	 */
	/*
	 * @ requires src != null; requires 0 <= index && index < src.m_Values.size(); ensures isString() && 0 <= \result && \result <
	 ...
	 */
	/*
	 * @ requires value != null; ensures isString() && 0 <= \result && \result < m_Values.size() || ! isString() && \result == -1;
	 */
	@SuppressWarnings("unchecked")
	 ...
	 * 
	 * @param date
	 *            the date, represented in milliseconds since January 1, 1970, 00:00:00 GMT, to return as string
	 ...
	 * @return the index of the given attribute value if attribute is nominal or a string, -1 if it is not or the value can't be found
	 */
	...
	public final/* @ pure @ */boolean isInRange(double value) {
		...
		if (m_Type == DATE || Instance.isMissingValue(value))
			return true;
		if (m_Type != NUMERIC) {
	 ...
	 * 
	 * ORDERING_SYMBOLIC - attribute values should be treated as symbols. ORDERING_ORDERED - attribute values have a global ordering.
	...
	/**
	 * Sets the metadata for the attribute. Processes the strings stored in the metadata of the attribute so that the properties can be set
	 * up for the easy-access metadata methods. Any strings sought that are omitted will cause default values to be set.
	 * 
	 * The following properties are recognised: ordering, averageable, zeropoint, regular, weight, and range.
	 * 
	 * All other properties can be queried and handled appropriately by classes calling the getMetadata() method.
	 * 
	 * @param metadata
	 *            the metadata
	 * @throws IllegalArgumentException
	...
	private void setMetadata(ProtectedProperties metadata) {
		...
		if (m_Type == DATE) {
			...
			String def;
			if (m_Type == NUMERIC && orderString.compareTo("modulo") != 0 && orderString.compareTo("symbolic") != 0)
				def = "true";
			else
				def = "false";
	...
	/**
	 * Sets the numeric range based on a string. If the string is null the range will default to [-inf,+inf]. A square brace represents a
	 * closed interval, a curved brace represents an open interval, and 'inf' represents infinity. Examples of valid range strings:
	...
	private void setNumericRange(String rangeString) {
		...
		if (m_UpperBound < m_LowerBound)
			throw new IllegalArgumentException("Upper bound (" + m_UpperBound + ") on numeric range is" + " less than lower bound ("
					+ m_LowerBound + ")!");
	}
	...
	/**
	 * Returns a description of this attribute in ARFF format. Quotes strings if they contain whitespace characters, or if they are a
	 * question mark.
	 * 
	...
	/**
	 * Returns a value of a nominal or string attribute. Returns an empty string if the attribute is neither a string nor a nominal
	 * attribute.
	 * 
	 * @param valIndex
	 *            the value's index
