 ...
 * </code>
 * <P>
 * or combinations thereof. The range is internally converted from 1-based to 0-based (so methods that set or get numbers not in string
...
public class Range implements Serializable, RevisionHandler {
	 ...
	 * 
	 * @param indices
	 *            an array containing indices to select. Since the array will typically come from a program, indices are assumed from 0, and
	...
	/* @non_null spec_public@ */Vector<String> m_RangeStrings = new Vector<String>();
	...
	/**
	 * Gets whether the range sense is inverted, i.e. all <i>except</i> the values included by the range string are selected.
	 * 
	...
	public/* @non_null pure@ */String getRanges() {
		...
		for (int i = 0; i < m_RangeStrings.size(); i++) {
			if (first) {
				result.append(m_RangeStrings.elementAt(i));
				first = false;
			} else {
				result.append(sep + m_RangeStrings.elementAt(i));
			}
		}
	...
	public/* @non_null@ */int[] getSelection() {
		...
		if (m_Invert) {
			...
			Enumeration<String> enu = m_RangeStrings.elements();
			while (enu.hasMoreElements()) {
				String currentRange = enu.nextElement();
	...
	/**
	 * Determines if a string represents a valid index or simple range. Examples: <code>first  last   2   first-last  first-4  4-last</code>
	...
	/**
	 * Sets whether the range sense is inverted, i.e. all <i>except</i> the values included by the range string are selected.
	 * 
	 * @param newSetting
	...
	/**
	 * Sets the ranges from a string representation. Note that setUpper() must be called afterwards for ranges to be actually set
	 * internally.
	 * 
	 * @param rangeList
	 *            the comma separated list of ranges. The empty string sets the range to empty.
	 * @throws IllegalArgumentException
