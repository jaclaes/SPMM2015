...
public class AttributeLocator implements Serializable, Comparable<AttributeLocator>, RevisionHandler {
	 ...
	 * less than, equal to, or greater than the specified object. Only type and indices are checked.
	 * 
	 * @param o
	 ...
	 * 
	 * @param findAtts
	 *            if true the indices of attributes are located, otherwise the ones of AttributeLocator objects
	...
	protected int[] find(boolean findAtts) {
		...
		if (findAtts) {
			for (i = 0; i < m_Attributes.size(); i++) {
				if (m_Attributes.get(i).booleanValue()) {
	...
	/**
	 * Returns the indices of the attributes. These indices are referring to the m_AllowedIndices array, not the actual indices in the
	 * Instances object.
	 * 
	...
	/**
	 * Returns the AttributeLocator at the given index. This index refers to the index of the m_AllowedIndices array, not the actual
	 * Instances object.
	 * 
	 * @param index
	...
	/**
	 * Returns the indices of the AttributeLocator objects. These indices are referring to the m_AllowedIndices array, not the actual
	 * indices in the Instances object.
	 * 
	 ...
	 * 
	 * @param data
	 *            the data to base the search for attributes on
	 * @param type
	 *            the type of attribute to look for
	 * @param indices
	 *            the indices that are allowed to check
	 */
