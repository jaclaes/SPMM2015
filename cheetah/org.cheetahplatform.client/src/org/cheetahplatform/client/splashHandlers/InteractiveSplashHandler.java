package org.cheetahplatform.client.splashHandlers;

import javax.jms.JMSException;

import org.cheetahplatform.client.SessionService;
import org.cheetahplatform.client.UiUtils;
import org.cheetahplatform.client.jms.JmsService;
import org.cheetahplatform.client.jms.LoginService;
...
public class InteractiveSplashHandler extends AbstractSplashHandler {
	...
	public InteractiveSplashHandler() {
		...
		// initialize JMS
		JmsService.getInstance();
	}

	private void authenticate(String username, String password) {
		try {
			LoginService loginService = new LoginService(username, password);
			loginService.synchronousRequest();
			if (loginService.getStatus().getSeverity() == IStatus.OK) {
				SessionService.setSessionId(loginService.getSessionId());
				fAuthenticated = true;
			} else {
				UiUtils.showError("Error when logging in to server. Reason: " + loginService.getStatus());
			}
		} catch (JMSException e) {
			UiUtils.showError("Could not login", e);
		}
	}
	...
	private void createUI() {
		...
		fTextUsername.setText("Jakob");
		fTextPassword.setText("börner");
	}
