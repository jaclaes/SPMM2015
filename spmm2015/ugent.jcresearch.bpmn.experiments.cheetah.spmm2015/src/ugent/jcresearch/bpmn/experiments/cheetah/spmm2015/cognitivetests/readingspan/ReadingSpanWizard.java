...
public class ReadingSpanWizard extends Wizard {
	...
	private List<List<ReadingSpanExercise>> exercises;
	private List<ReadingSpanExercise> demos;
	private PromLogger logger;
	private LoggingValidator logValidator;

	public static final String READING_SPAN = "ReadingSpan";
	...
	public ReadingSpanWizard(List<ReadingSpanExercise> demos, List<List<ReadingSpanExercise>> exercises, PromLogger logger) {
	...
	public void addPages() {
		addPage(new TestInstructionsPage(INSTRUCTIONS, 
				...
				+ "This is a test of your ability to assess the possible validity of simple statements while "
				+ "remembering short words at the same time.\n\n"
				+ "WHICH ASSIGNMENTS DO I HAVE TO COMPLETE?\n\n"
				+ "One by one you will be presented simple statements of which you have to assess if they make "
				+ "sense or not. After each statement a word you will have to remember is displayed.\n\n"
				+ "After a series of statements you will be asked to list the remembered words of the series in "
				+ "the correct serial order.\n\n"
				+ "The length of a series varies from 2 to 6 statements, with three trials of each series size, "
				+ "but in a random order (you will not know upfront the length of the current series you are working on).", 1, 3));
		
		addPage(new TestInstructionsPage(INSTRUCTIONS,
				"HOW SHOULD I APPROACH THIS TEST?\n\n"
				+ "For an accurate test result, it is advised to read the statement aloud. Then read aloud the "
				+ "word to remember. Pronounce the answer (either 'makes sense' or 'makes no sense') as you "
				+ "press the appropriate button.\n\n"
				+ "You are not supposed to take time to rehearse the words by repeating the series of words you "
				+ "remembered so far. Therefore, make sure to maintain a steady tempo while reading and assessing "
				+ "the assignments.\n\n"
				+ "If you are not alone, try to read the line silently but explicitly as if you would read it aloud "
				+ "in your head.\n\n"
				+ "HOW IS THE SCORE CALCULATED?\n\n"
				+ "Your reading span score is the cumulative number of words recalled from perfectly recalled trials. "
				+ "This means you will have to list every word of the series in the correct serial order.", 2, 3));
		...
		TestInstructionsPage ipage = new TestInstructionsPage(INSTRUCTIONS, 
				...
				+ "When cheating is detected, no score will be calculated and you will be excluded from the experiment.\n\n"
				...
				+ "as soon as possible and mention the details of the distraction in the feedback box at the end of "
				+ "the test.\n\n"
				+ "First some demo statements will be presented.", 3, 3);
		...
		for (int i = 0; i < demos.size(); i++) {
			ReadingSpanWizardPage wpage = new ReadingSpanDemoWizardPage(READING_SPAN, demos, i);
			wpage.addLogListener(logValidator);
			addPage(wpage);
		}
		ReadingSpanResultsPage page = new ReadingSpanDemoResultsPage(RESULTS, demos);
		...
		int num = 1;
		for (List<ReadingSpanExercise> level : exercises) {
			for (int i = 0; i < level.size(); i++) {
				ReadingSpanWizardPage wpage = new ReadingSpanWizardPage(READING_SPAN, level, i, num, exercises.size()); 
				wpage.addLogListener(logValidator);
				addPage(wpage);
			}
			page = new ReadingSpanResultsPage(RESULTS, level, num, exercises.size());
			page.addLogListener(logValidator);
			addPage(page);
			num++;
		}
		...
		ipage = new TestInstructionsPage(INSTRUCTIONS, 
				"This is the end of the Reading Span Test. Thank you for your effort and concentration!", 0, 0);
		ipage.addFocusGainedListener(new IFocusGainedListener() {
			public void onFocusGained() {
				AuditTrailEntry entry = new AuditTrailEntry(RESULT);
				entry.setAttribute(new Attribute("Reading Span Score", TestScorePage.score));
		...
		if (wizardContainer instanceof IPageChangeProvider) {
			...
			pageChangeProvider.addPageChangedListener(new IPageChangedListener() {
				...
				public void pageChanged(PageChangedEvent event) {
					...
					if (prevPage != null
						&& prevPage instanceof ReadingSpanWizardPage)
						((ReadingSpanWizardPage)prevPage).setCanAutoFlip(false);
					if (page.getName().equals(READING_SPAN)) {
						...
						timer.schedule(new TimerTask() {
							...
							public void run() {
								Display.getDefault().asyncExec(new Runnable() {
									@Override
									public void run() {
										if (page instanceof ReadingSpanWizardPage 
										    && !((ReadingSpanWizardPage)page).canAutoFlip())
											return;
						...
						((ReadingSpanResultsPage) page).logInitialStatus();
					}
				}
			});
		}
	...
	protected void logPageChanged(IWizardPage wizardPage) {
		if (wizardPage.getName().equals(READING_SPAN)) {
			ReadingSpanWizardPage page = (ReadingSpanWizardPage) wizardPage;
			// log new level
