...
public abstract class Javadoc implements OptionHandler, RevisionHandler {
	 ...
	 * 
	 * @param javadoc
	 *            the javadoc producer to execute
	 * @param options
	...
	/**
	 * generates either the plain Javadoc (if no filename specified) or the updated file (if a filename is specified). The start and end tag
	 * for the global info have to be specified in the file in the latter case.
	 * 
	 ...
	 * 
	 * @param index
	 *            the index in the start/end tag array
	 * @return the generated Javadoc
	 * @throws Exception
	 *             in case the generation fails
	 */
	...
	/**
	 * determines the base string of the given indention string, whether it's either only spaces (one space will be retured) or mixed mode
	 * (tabs and spaces, in that case the same string will be returned)
	 * 
	 * @param str
	 *            the string to analyze
	 ...
	 * @param count
	 *            the number of times to indent one line
	 * @param indentStr
